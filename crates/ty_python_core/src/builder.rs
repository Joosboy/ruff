        ...
        match stmt {
            ...
            ast::Stmt::ImportFrom(node) => {
                ...
                let package_program_file = self
                    .file
                    .is_package(self.db)
                    .then(|| self.analysis_file.program_file(self.db));
                if let Some(program_file) = package_program_file
                    ...
                    && let Ok(thispackage) = ModuleName::package_for_file(self.db, program_file)
                {
                ...
                for (alias_index, alias) in node.names.iter().enumerate() {
                    if &alias.name == "*" {
                        ...
                        let Ok(module_name) =
                            ModuleName::from_import_statement(self.db, program_file, node)
                        else {
                            continue;
                        };
