...
impl<'db> ScopeId<'db> {
    ...
    pub fn scope(self, db: &'db dyn Db) -> &'db Scope {
        semantic_index(db, self.analysis_file(db)).scope(self.file_scope_id(db))
    }
    ...
    pub fn class_definition_of_method(self, db: &'db dyn Db) -> Option<Definition<'db>> {
        semantic_index(db, self.analysis_file(db))
...
impl FileScopeId {
    ...
    pub fn to_scope_id<'db>(self, db: &'db dyn Db, file: AnalysisFile<'db>) -> ScopeId<'db> {
