...
use crate::place::{
    ...
    TypeOrigin, builtins_module_scope, imported_symbol, known_module_symbol,
};
...
pub fn check_types(
    ...
    let diagnostics = check_suppressions(db, analysis_file.versioned_file(db), diagnostics);
                        ...
                        }) =>
                {
                    ...
                    (match name_str {
                        "name" if is_enum_subclass => {
                            enum_class.name_type(db, program, enum_literal.name(db))
                        }
                        "_name_" => enum_class.name_type(db, program, enum_literal.name(db)),
...
impl<'db> ModuleLiteralType<'db> {
    ...
    fn try_module_getattr(self, db: &'db dyn Db, name: &str) -> PlaceAndQualifiers<'db> {
        ...
        if let Some(analysis_file) = self.module_analysis_file(db) {
            let getattr_symbol = imported_symbol(
                db,
                ImportedSymbolContext::File(analysis_file),
                "__getattr__",
                None,
            );
    ...
    fn static_member(self, db: &'db dyn Db, name: &str) -> PlaceAndQualifiers<'db> {
        ...
        let context = self.module_analysis_file(db).map_or(
