...
mod resolve_definition {
    ...
    pub(crate) fn resolve_from_import_definitions<'db>(
        ...
        let program_file = analysis_file.program_file(db);
        let Some(module_name) =
            ModuleName::from_import_statement(db, program_file, import_node).ok()
        else {
            return Vec::new();
        };
        let Some(resolved_module) = resolve_module(db, program_file, &module_name) else {
            return Vec::new();
        };
