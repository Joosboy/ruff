...
impl<'db, 'ast> TypeInferenceBuilder<'db, 'ast> {
    ...
    fn report_unresolved_import(
        ...
        if level == 0 {
            ...
            let program_file = self.analysis_file().program_file(db);
            if let Some(better_level) = (0..level).rev().find(|reduced_level| {
                let Ok(module_name) =
                    ModuleName::from_identifier_parts(db, program_file, module, *reduced_level)
                else {
                    return false;
                };
                ty_module_resolver::resolve_module(db, program_file, &module_name).is_some()
            }) {
                diagnostic
    ...
    fn check_import_from_module_is_resolvable(&mut self, import_from: &ast::StmtImportFrom) {
        ...
        let program_file = self.analysis_file().program_file(db);
        let module_name = ModuleName::from_import_statement(db, program_file, import_from);
        ...
        if ty_module_resolver::resolve_module(db, program_file, &module_name).is_none() {
