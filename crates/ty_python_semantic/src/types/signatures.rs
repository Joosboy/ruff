...
impl<'c, 'db> TypeRelationChecker<'_, 'c, 'db> {
    ...
    fn check_signature_pair_with_quantified_locals(
        ...
        for typevar in source_typevars {
            ...
            quantified =
                with_domain.exists(db, self.constraints, std::iter::once(typevar.identity(db)));
        }
        ...
        let target_identities = target_typevars
            .iter()
            .map(|typevar| typevar.identity(db))
            .collect::<Vec<_>>();
        let universally_quantified = target_domain.implies(db, self.constraints, || quantified);
        let Some(result) =
            universally_quantified.try_for_all(db, self.constraints, target_identities)
        else {
            return self.never();
        };
        result
